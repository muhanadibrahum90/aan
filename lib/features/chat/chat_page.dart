...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<String?> _uploadImage(File image) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('https://an-upload.an52907394.workers.dev/upload-chat'),
        headers: {
  ...
  Future<void> _deleteFromR2(String key) async {
    try {
      final token = await FirebaseAuth.instance.currentUser!.getIdToken();
      await http.post(
        Uri.parse('https://an-upload.an52907394.workers.dev/delete-chat-image'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
...
class _FullScreenImageState extends State<_FullScreenImage> {
  ...
  Future<void> _downloadImage() async {
    ...
    try {
      ...
      final filePath =
          '${tempDir.path}/an_${DateTime.now().millisecondsSinceEpoch}.jpg';
